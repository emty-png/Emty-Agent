...
function renderToolEvent(event: ToolEvent): string {
  ...
  if (resultPreview) lines.push(`Result preview:\n${resultPreview}`)
  return lines.join('\n')
}
...
function prePassMessages(messages: Message[]): Message[] {
  return messages
    ...
    .map(m => {
      ...
      if (m.attachments) copy.attachments = [...m.attachments]
      if (m.toolEvents) copy.toolEvents = [...m.toolEvents]
      
      // Strip historical media
      if (copy.attachments) {
        copy.attachments = copy.attachments.map(att => {
          const a = att as any
          if (a.contentType?.startsWith('image/') && a.url?.startsWith('data:')) {
            return { ...att, url: '', name: `[Image stripped during compaction: ${a.name}]` }
          }
          return att
        })
      }
...
function renderMessageForCompaction(message: Message): string {
  ...
  if (message.mentionContext?.trim()) blocks.push(`Mention context:\n${truncateText(message.mentionContext, 1200)}`)
  if (message.attachments?.length) blocks.push(`Attachments: ${message.attachments.map(item => item.name).join(', ')}`)
  if (message.toolEvents?.length) {
    const renderedEvents = message.toolEvents.map(renderToolEvent).join('\n\n')
    if (renderedEvents.trim()) blocks.push(renderedEvents)
  }
  return blocks.join('\n\n').trim()
}
...
function calculateCompactionBoundaries(messages: Message[], budgetTokens: number): { head: Message[], toCompact: Message[], tail: Message[] } {
  ...
  // Head protection
  const systemMessages = realMessages.filter(m => m.role === 'system' as any)
  const nonSystem = realMessages.filter(m => m.role !== 'system' as any)
  ...
  let remaining = nonSystem.slice(protectCount)
  
  // Tail protection
  let tailProtected: Message[] = []
  let currentTokens = 0
  ...
  let toCompact = remaining.slice(0, i + 1)
...
function buildCompactionModel(activeModel: ActiveModelSnapshot, settings: SettingsSnapshot): LanguageModel {
  ...
  if (!compatible) throw new Error(`Provider "${activeModel.providerId}" not found`)
...
async function buildSessionSummary(messagesToCompact: Message[], tab: ChatTab, settings: SettingsSnapshot, previousSummary?: string, focusTopic?: string): Promise<CompactionSummaryResult> {
  ...
  if (!activeModel) throw new Error('No active model selected for session compaction')
...
export function shouldCompactSession(tab: ChatTab, thresholdPercent: number): boolean {
  if (tab.isCompacting) return false
...
export async function compactConversationSession(options: {
  ...
  if (toCompact.length === 0) throw new Error('Not enough conversation history to compact')
  ...
  if (!options.tab.compactionStats) options.tab.compactionStats = { lastSavingsPct: 0, lastCompactedAt: 0 }
